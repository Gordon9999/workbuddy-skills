...
## 6. 正文段落与换行（三者一致，用户强要求，违反即不合格）

**换行不是排版细节，是内容结构。** 每条正文都必须满足：

1. **一个意思 = 一个 `<p>` = 一次换行**。禁止把多个意思塞进同一个 `<p>`。
2. **一段最多 3 句**。超过 3 句必拆；金句 / 判断句 / 关键数据句可单句成段。
3. **禁止「一大坨」**：连续 4 句以上中间没有任何换行 ＝ 不合格。
4. `<br>` 只用于**比段落更大的间隔**，不当换行工具用；段间留白由 `<p>` 的 margin 负责。
5. Markdown / 纯文本输出同理：**段与段之间必须空一行**。

```css
.article-body p {
    line-height: 1.8;
    margin-bottom: 1.05rem;   /* 段间距，不可调小 */
}
.article-body br {
    display: block;
    margin: 0.45rem 0;
...
生成/改写完成后**逐段自查**：数每段句子数，超 3 句就拆，拆完再交付。
...
- 改基线值（颜色、字体、容器宽度、高亮、底部、换行、断点）→ 只改本文件 → 同步到三个模板的内联 CSS。
